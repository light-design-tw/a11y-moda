...
## Part B — 110.07 移除 18 條：對帳結果（已執行）

以 a11y-moda 實作集 × 18 移除碼三方比對。**SC-match 啟發法不可靠**（例：GN1210101E 與 FA1210102E 同 SC 2.1.1 但檢查不同），故逐條讀規則邏輯判定。

- **11 條從未實作 → 無動作**，含唯一確定真廢 **HM1410100C**（WCAG 2.2 移除 4.1.1 Parsing）。
- **3 條孤兒有乾淨 115.11 對應 → 已改名**（目標皆未實作，改名不產生重複）：

| 舊碼 | 新碼 | SC | 備註 |
|---|---|---|---|
| HM3330500C | GN3330502E | 3.3.5 | title 脈絡協助 |
| FA2141104E | FA2240701E | 2.4.7 | outline:none 無 :focus（含 lint 同步改名）|
| GN2141100E | GN3241302E | 1.4.11→2.4.13 | 作者焦點框線（AA→AAA、移至 focus/）|

> 意外收穫：FA2240701E、GN3241302E 原列「暫緩/待寫」，實為既有孤兒改名即得 — 不需新寫。

- **4 條孤兒無 115.11 對應碼、檢查仍有效 → 保留為 legacy 110.07**（刪除會掉覆蓋）：

| 保留碼 | SC | 檢查（與哪條沿用碼不同）|
|---|---|---|
| GN1210101E | 2.1.1 | tabindex=-1 從 Tab 序移除（≠ 保留的 GN1210100E 滑鼠/鍵盤事件等效）|
| GN1240301E | 2.4.3 | runtime 焦點順序 / dialog focus trap（≠ GN1240300E 靜態 tab-pattern）|
| HM1110108E | 1.1.1 | object 替代內容完整性（LLM）|
| HM1130108E | 1.3.1 | 資料表格 caption 描述性（LLM）|
...
| GN3241302E | 2.4.13 | 🟢 DOM | ✅ **改名自既有孤兒 GN2141100E**（Part B）— 偵測作者 :focus CSS 樣式（outline/box-shadow/border/background）。AA→AAA、移至 focus/ |
| FA2240701E | 2.4.7 | 🟢 DOM | ✅ **改名自既有孤兒 FA2141104E**（Part B）— `<style>` outline:none 無 :focus 替代 → fail（scan + lint 雙軌）|
| GN3330502E | 3.3.5 | 🟢 DOM | ✅ **改名自既有孤兒 HM3330500C**（Part B）— title 脈絡協助（衍生規則）|
